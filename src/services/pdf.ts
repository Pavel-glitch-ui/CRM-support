...
export async function generateAuditPdf(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor(redColor).font(fontBold).fontSize(11).text(`${metrics.summary.stuckDealsCount} шт.`, col3, startY + 32);
      doc.fillColor(darkColor).font(fontRegular).fontSize(8).text(`Просрочек: ${metrics.tasks.overduePercent}%`, col3, startY + 48);
